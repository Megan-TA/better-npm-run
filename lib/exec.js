...
module.exports = function exec(script, program, resetEnvParams) {
  ...
  var resetEnvObj = resetEnvParams.reduce(function (previous, current) {
    const temp = current.split("=")
    previous[temp[0]] = temp[1]
    return previous
  }, {})

  var env = objectAssign({}, process.env, script.env, resetEnvObj),
      sh = 'sh',
      shFlag = '-c';
